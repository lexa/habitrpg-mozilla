...
var is_penalty_time = function (time){
    pentime = Prefs.penaltytime.split("-");
    return (parseInt(pentime[0])<=time&&time<=parseInt(pentime[1]));
...
exports.main = function()
{
    ...
    tabs.on('open', function(tab){
        tab.on('ready', function(tab){
            ...
            if (is_vice_host(host)&&is_penalty_time(current_time.getHours())) {
                //punish user
                notifications.notify({
                    title: _("HabitRPG"),
                    text:  _("notification_text")
                })
